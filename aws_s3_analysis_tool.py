...
def main():
    ...
    parser.add_argument('--service', help='AWS service', default='s3')
    ...
    parser.add_argument('--uri', default='https://s3.amazonaws.com')
    ...
    request_list = make_request(args.request,
                     args.service,
                     args.region,
                     args.uri,
                     headers,
    ...
    root = ET.fromstring(request_list.content)
    my_buckets = {}

    for bucket in root.iter(NS+'Bucket'):
        my_buckets[bucket.find(NS+'Name').text] = bucket.find(NS+'CreationDate').text

    request_list.raise_for_status()

    for key in my_buckets:
        request_region = make_request('GET',
                     's3',
                     args.region,
        ...
        root2 = ET.fromstring(request_region.content)
        region = root2.find('Region').text
        request_bucket_info = make_request(args.request,
                     args.service,
                     region,
                     "https://"+ key + ".s3.amazonaws.com/?list-type=2",
                     headers,
                     data,
                     args.access_key,
                     args.secret_key,
        ...
        root3 = ET.fromstring(request_bucket_info.content)
        num_files = root3.find(NS+'KeyCount').text
        size_files = 0;
        for content in root3.iter(NS+'Contents'):
             size_files += float(content.find(NS+'Size').text)
        print("Bucket name: " + key + "\n Creation date: " + my_buckets[key] + "\n Number of files: " + num_files + "\n Total Size in Mb: " + str(round(size_files/1024, 2)))
